  ...
  for (let i = 0; i < embeds.length; i++) {
    try {
      ...
      if (html) {
        ...
        finalPage = finalPage.replace(embeds[i].string, `<div class="mb-14">${html}</div>`);
      } else {
        const section = `<div><a class="border border-accent rounded-md mb-6 p-6" href="${embeds[i].url}" target="_blank" rel="noreferrer">${result.title}</a></div>`;
