...
export async function getModifiedPage(page) {
  ...
  for (let i = 0; i < embeds.length; i++) {
    try {
      ...
      console.log("file: api.js ~ line 41 ~ getModifiedPage ~ result", JSON.stringify(result));
        ...
        <div className="mb-14">
          <div className="text-white bg-gray-900 px-6 py-3 rounded-md">
            <a className="" href="${embeds[i].url}" target="_blank" rel="noreferrer">
              <p className="text-white font-bold mb-1">${result?.twitter_card?.title || result?.title}</p>
              <p className="text-white text-sm">${result?.twitter_card?.description}</p>
            </a>
          </div>
        </div>`;
